...
export const api = {
  ...
  taskGet: (taskId: string) => call<TaskCard>("task_get", { task_id: taskId }),
  taskCreate: (task: TaskCard) => call<TaskCard>("task_create", { task }),
  taskUpdate: (task: TaskCard) => call<TaskCard>("task_update", { task }),
  taskImport: (payload: string) => call<TaskCard[]>("task_import", { payload }),
  taskArchive: (taskId: string) => call<TaskCard>("task_archive", { task_id: taskId }),
  taskRoutePreview: (taskId: string) => call<RoutingDecision>("task_route_preview", { task_id: taskId }),
  taskEnqueue: (taskId: string) => call<TaskCard>("task_enqueue", { task_id: taskId }),
  taskRunNow: (taskId: string) => call<TaskCard | null>("task_run_now", { task_id: taskId }),
  taskCancel: (taskId: string) => call<TaskCard>("task_cancel", { task_id: taskId }),
  taskRetry: (taskId: string) => call<TaskCard>("task_retry", { task_id: taskId }),
  taskApprove: (taskId: string, feedback?: string) => call<TaskCard>("task_approve", { task_id: taskId, feedback }),
  taskReject: (taskId: string, feedback?: string) => call<TaskCard>("task_reject", { task_id: taskId, feedback }),
  taskFeedback: (taskId: string, feedback: string) => call<TaskCard>("task_feedback", { task_id: taskId, feedback }),
  taskArtifacts: (taskId: string) => call<TaskArtifact[]>("task_artifacts", { task_id: taskId }),
  taskArtifactRead: (taskId: string, path: string) => call<string>("task_artifact_read", { task_id: taskId, path }),
  taskArtifactOpen: (taskId: string) => call<void>("task_artifact_open", { task_id: taskId }),

  orchestratorStatus: () => call<OrchestratorStatus>("orchestrator_status"),
  orchestratorStart: () => call<OrchestratorStatus>("orchestrator_start"),
  orchestratorPause: () => call<OrchestratorStatus>("orchestrator_pause"),
  orchestratorResume: () => call<OrchestratorStatus>("orchestrator_resume"),
  orchestratorStop: () => call<OrchestratorStatus>("orchestrator_stop"),
  orchestratorRunOnce: () => call<TaskCard | null>("orchestrator_run_once"),
  workerHealth: () => call<WorkerHealth[]>("worker_health"),
  modelCapabilities: () => call<ModelCapability[]>("model_capabilities"),
  modelRefresh: () => call<ModelCapability[]>("model_refresh"),
